...
$data['field_id'] = "";
$data['bypassCsrf'] = (isset($post['bypassCsrf']) ? intval($post['bypassCsrf']) : "0");
...
if ($data['action'] == "new") {
    ...
    /* Verificação de CSRF */
    if (!csrf_verify($post) && !$data['bypassCsrf']) {
        $data['success'] = false; 
    ...
    try {
        ...
        */
        ...
        /* Atualização da etiqueta de referência para componentes avulsos */
        ...
        /* Final da atualização da localização dos componentes e da etiqueta de referencia */
            ...
            if ($recordFile && $recordFile[$i]) {
                ...
                if (chop($fileinput) != "") {
                    ...
                    $sqlFile = "INSERT INTO imagens (img_nome, img_inst, img_inv, img_tipo, img_bin, img_largura, img_altura, img_size) values " .
                        "('" . noSpace($anexo['name'][$i]) . "', '" . $data['asset_unit'] . "', '" . $data['asset_tag'] . "','" . $anexo['type'][$i] . "', " .
