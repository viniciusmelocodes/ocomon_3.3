...
$sql = "SELECT sis_id, sistema FROM sistemas WHERE sis_atende = 1 {$filter_areas} ";
...
foreach ($result->fetchAll() as $row) {
    ...
    foreach ($datesBegin as $dateStart) {
        ...
        $sqlEach = "SELECT 
                    ...
                    FROM 
                        ocorrencias o, sistemas s 
                    WHERE 
                        s.sis_id = o.sistema 
                        AND s.sis_id = " . $row['sis_id'] . " 
                        AND o.data_fechamento >= '" .  $dateStart  . "' 
                        AND o.data_fechamento <= '" .  $datesEnd[$i]  . "' 
                    GROUP BY sistema
                    ";
